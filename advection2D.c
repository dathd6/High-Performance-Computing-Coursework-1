...
#pragma omp parallel for collapse(2) shared(u)
   ...
   *      different threads is mixed together lead to corrupted file or data is
   *      not in ordered
   * */
  for (int i = 0; i < NX + 2; i++) {
    for (int j = 0; j < NY + 2; j++) {
  ...
  for (int m = 0; m < nsteps; m++) {
    ...
    /* LOOP 6 */
#pragma omp parallel for private(t2, y2) shared(u, dt, y, m)
    for (int j = 0; j < NY + 2; j++) {
      /* Task 2:
...
#pragma omp parallel for collapse(2) shared(u, dudt, dt)
  ...
  float vert_avg_dist[NX]; /* array store vertically averaged values of u at
                              each value of x */
